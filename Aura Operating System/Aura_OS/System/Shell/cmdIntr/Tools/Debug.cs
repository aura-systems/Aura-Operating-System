        ...
        public static void c_Debug(string settings)
        {
            ...
            else if (cmdargs.Length == 2) //No arg
            {
                if (cmdargs[1].Equals("vbeinfo"))
                {
                    ...
                    DebugConsole.WriteLine("VBE Pointer: 0x" + Conversion.DecToHex((int)Graphics.VBE.Graphics.ModeInfo.framebuffer));
                    DebugConsole.WriteLine("VBE Mode: " + Graphics.VBE.Graphics.VESAMode);

                    DebugConsole.WriteLine();

                    DebugConsole.WriteLine("[VBE Controller Information]");
                    
                    DebugConsole.WriteLine("VBE Signature: " + Graphics.VBE.Graphics.VBESignature);

                    DebugConsole.WriteLine("VBE Version: " + Graphics.VBE.Graphics.VBEVersion);

                    DebugConsole.WriteLine("OEM String: " + Graphics.VBE.Graphics.VBEOEM);

                    DebugConsole.WriteLine("Capabilites: " + Graphics.VBE.Graphics.ControllerInfo.capabilities);

                    DebugConsole.WriteLine("Total Memory: " + (Graphics.VBE.Graphics.ControllerInfo.totalmemory * 64) + " kB");

                    DebugConsole.WriteLine("OEM Software Rev: " + Graphics.VBE.Graphics.ControllerInfo.oemSoftwareRev);

                    DebugConsole.WriteLine("Video RAM Pointer: 0x" + Conversion.DecToHex((int)Graphics.VBE.Graphics.ModeInfo.framebuffer));

                    DebugConsole.WriteLine("OEM String Pointer: 0x" + Conversion.DecToHex((int)Graphics.VBE.Graphics.ControllerInfo.oemStringPtr));

                    DebugConsole.WriteLine("OEM Vendor Name Pointer: 0x" + Conversion.DecToHex((int)Graphics.VBE.Graphics.ControllerInfo.oemVendorNamePtr));
